...
/* ---------- Reuse the same Card styles as ProductsPage ---------- */
...
const CategoryPage = () => {
  ...
  const originalCategoryName = useMemo(() => {
  ...
  const categoryProducts = useMemo(() => {
    if (!originalCategoryName) return [];
    return items.filter(item => 
      item.category && 
      item.category.toLowerCase() === originalCategoryName.toLowerCase()
    );
  }, [items, originalCategoryName]);

  const totalPages = Math.max(1, Math.ceil(categoryProducts.length / PAGE_SIZE));
  const clampedPage = Math.min(Math.max(currentPage, 1), totalPages);
  const start = (clampedPage - 1) * PAGE_SIZE;
  const pageItems = categoryProducts.slice(start, start + PAGE_SIZE);
  ...
  return (
    <main className="bg-white">
      <section className="mx-auto max-w-7xl px-4 sm:px-6 pt-10 pb-6">
        <div className="flex items-end justify-between gap-3">
          <div>
            ...
            <h1 className="mt-3 text-[28px] md:text-[34px] leading-tight text-slate-800">
              {originalCategoryName} Products
            </h1>
            <p className="mt-1 text-slate-600 text-sm">
              {categoryProducts.length} products • Page {clampedPage} of {totalPages}
            </p>
          </div>
        </div>
        ...
        {/* Show message if no products found in this category */}
        {state !== "loading" && categoryProducts.length === 0 && (
          <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-8 text-center text-slate-600">
            <p className="text-lg font-medium">No products found in "{originalCategoryName}" category.</p>
            <Link 
              to="/shop-all" 
              className="mt-4 inline-flex items-center text-[#d8a298] hover:underline"
            >
              Browse all products →
            </Link>
          </div>
